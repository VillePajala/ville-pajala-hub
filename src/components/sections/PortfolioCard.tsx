...
export function PortfolioCard({ project, index }: PortfolioCardProps) {
  ...
  return (
    <motion.article
      ...
      className="group project-card overflow-hidden rounded-lg border border-white/10 h-full flex flex-col bg-gradient-to-br from-zinc-900/50 to-zinc-700/30 backdrop-blur-sm hover:border-white/30 hover:shadow-xl hover:shadow-zinc-900/20 hover:-translate-y-2 hover:scale-[1.03] hover:rotate-[0.3deg] transition-all duration-500 ease-out"
    >
      ...
      <div className="p-6 flex-grow flex flex-col bg-gradient-to-b from-zinc-800/20 via-zinc-700/25 to-zinc-600/30">
        ...
        <h3 className="mb-3 text-xl font-serif font-medium text-white/95 transition-transform duration-500 group-hover:translate-x-1">
          <Link href={`/portfolio/${project.slug}`} className="hover:text-white transition-colors duration-300">
            {project.title}
          </Link>
        </h3>
        ...
        <div className="flex items-center justify-between mt-auto pt-4 border-t border-white/10 transition-colors duration-500 group-hover:border-white/20">
          <div className="flex flex-wrap gap-1">
            {project.technologies.slice(0, 2).map((tech) => (
              <span 
                key={tech} 
                className="text-xs text-white/60 transition-colors duration-500 group-hover:text-white/75"
              >
                {tech}
                {project.technologies.indexOf(tech) < Math.min(1, project.technologies.length - 1) && ", "}
              </span>
            ))}
            {project.technologies.length > 2 && (
              <span className="text-xs text-white/60 transition-colors duration-500 group-hover:text-white/75">
